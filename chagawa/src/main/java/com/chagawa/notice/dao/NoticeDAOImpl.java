...
public class NoticeDAOImpl implements NoticeDAO {
	...
	// 현재 공지 리스트
		...
		public List<NoticeVO> currentList(PageObject pageObject) throws Exception {
			...
			try {
				...
				// 3. sql
				// 3-1. 원본 데이터 가져오기
				String sql = "select no, title, to_char(startDate, 'yyyy-mm-dd') startDate, to_char(endDate, 'yyyy-mm-dd') endDate, to_char(updateDate, 'yyyy-mm-dd') updateDate from notice where startDate <= sysdate and trunc(sysdate) <= endDate ";
				// 검색 조건 처리
				sql += searchSQL(pageObject);
	...
	public int updateImg(NoticeVO vo) throws Exception {
		...
		try {
			con = DB.getConnection();
			String sql = "update notice set imgFile = ? where no = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, vo.getImgFile());
			pstmt.setLong(2, vo.getNo());
			result = pstmt.executeUpdate();
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
			throw new Exception("이미지 수정 DB 처리 중 오류 발생");
		} finally {
			DB.close(con, pstmt);
		}
		return result;
	...
	public int updateGnr(NoticeVO vo) throws Exception {
		...
		try {
			con = DB.getConnection();
			String sql = "update notice set gnrFile = ? where no = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, vo.getGnrFile());
			pstmt.setLong(2, vo.getNo());
			result = pstmt.executeUpdate();
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
			throw new Exception("일반 파일 수정 DB 처리 중 오류 발생");
		} finally {
			DB.close(con, pstmt);
		}
		return result;
